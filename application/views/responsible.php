...
<div class="right_col" role="main">
  <div class="page-title">
    ...
    <h3>Mantenedor de responsables</h3>
    </div>
  </div>
      ...
      <div class="col-md-12">
        <div class="x_panel">
          <div class="x_content padded">
              <div class="row">
                <div class="col-md-12">
                <button  data-target='#viewUser' data-toggle='modal' type="button" class="pull-right btn btn-primary"><span class="glyphicon glyphicon-plus-sign"></span> Nuevo Responsable</button>
    ...
    //console.log(loadingDiv);
      ...
      $("#form").submit(function(){
         ...
         $.ajax({
            type:'get',
            dataType:'json',
            url:'<?=site_url("contingency/saveResponsible")?>/',
            data:formData,
            beforeSend:function(){
      ...
      $(document).on('click','.edit',function(){
        ...
        if(typeof id != 'undefined'){
          ...
          $.ajax({
            type:'POST',
            dataType:'json',
            url:'<?=site_url("contingency/findResponsible")?>/'+id,
            beforeSend:function(){
               $("body").css("cursor",'wait');
            },
            success:function(data){
              
              $("#name").val(data.responsible.res_name);
              $("#id").val(data.responsible.res_id);
              $("#status option[value='"+data.responsible.res_status+"'] ").prop('selected',true);
            },
            complete:function(){
               $("body").css("cursor",'default');
            }
